...
There are $$`format(sum(res05$padj < 0.05, na.rm = TRUE))`$$ DEG's between Smoker & Non-Smoker groups;
$$`format(sum(resCOPD$padj < 0.05, na.rm = TRUE))`$$ between COPD & Non-Smoker groups;
and $$`format(sum(resWild$padj < 0.05, na.rm = TRUE))`$$ between COPD & Smoker groups. 


### Log fold Skrinking and a chosen contract
...
pLFC <- plotMA(resLFC, ylim = c(-2, 2), main = "res Shruken effect size", returnData=TRUE)
